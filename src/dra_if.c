...
int
if_setsockopt_sctp_initmsg(int sd, struct sctp_initmsg *initmsg)
{
	int ret;
	...
	ret = setsockopt(sd, IPPROTO_SCTP, SCTP_INITMSG, initmsg, sizeof(struct sctp_initmsg));
	if (ret < 0) {
		log_message(LOG_INFO, "cant set SCTP_INITMSG SCTP option. errno=%d (%m)", errno);
		close(sd);
		return -1;
	}
...
int
if_setsockopt_sctp_events(int sd, struct sctp_event_subscribe *events)
{
	int ret;
	...
	ret = setsockopt(sd, IPPROTO_SCTP, SCTP_EVENTS, events, sizeof(struct sctp_event_subscribe));
	if (ret < 0) {
		log_message(LOG_INFO, "cant set SCTP_EVENTS SCTP option. errno=%d (%m)", errno);
		close(sd);
		return -1;
	}
...
int
if_setsockopt_sctp_nodelay(int sd, int onoff)
{
	int ret;
	...
	ret = setsockopt(sd, IPPROTO_SCTP, SCTP_NODELAY, &onoff, sizeof(onoff));
	if (ret < 0) {
		log_message(LOG_INFO, "%s(): cant set SCTP_NODELAY (%m)"
				    , __FUNCTION__);
		close(sd);
		return -1;
	}
...
int
if_getsockopt_sctp_status(int sd, struct sctp_status *status)
{
	socklen_t len = sizeof(struct sctp_status);

	if (sd < 0)
		return -1;

	return getsockopt(sd, IPPROTO_SCTP, SCTP_STATUS, status, &len);
}
